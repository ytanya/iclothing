...
namespace iClothing
{
    public partial class ProductManagment : UserControl
    {
        ...
        private string currentOrderByItem = "Kihieu";
        ...
        private void ProductManagment_Load(object sender, EventArgs e)
        {
            ...
            {
                //string query = "Select `Barcode,`Ten`.`MieuTa`,`NgayTao`,`NgaySua` from `Color`";
            }
        }
        ...
        private void dvgArt_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.ColumnIndex == 5)
            {
                ...
                {
                    string query = "UPDATE ART SET Ten = '" + Ten + "',Mota = '" + Mieuta + "',Ngaytao = '" + now + "',Ngaysua = '" + now + "' WHERE Barcode= " + ARTID;
                    bool isSuccess = DBAccess.ExecuteQuery(query);
                    if (isSuccess)
                    {
                        PopulateData(currentPageNumber, rowPerPage, currentOrderByItem);
